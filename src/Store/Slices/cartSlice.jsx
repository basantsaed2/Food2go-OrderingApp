...
// Calculate detailed tax breakdown for an item (PRODUCT AND ADDONS ONLY)
const calculateItemTaxDetails = (product, variations, addons, quantity) => {
  ...
  // Variation taxes (considered part of product)
  if (variations) {
  ...
  // Addon taxes (ONLY ADDONS GET TAXED)
  if (addons) {
    Object.entries(addons).forEach(([addonId, addonData]) => {
      if (addonData.checked) {
        ...
        if (addon) {
          ...
          taxDetails.taxableAmount += parseFloat(addon.price) * quantity * addonQty;
          taxDetails.taxBreakdown.push({
            name: `${product.name} - ${addon.name}`,
            type: 'addon',
            taxableAmount: parseFloat(addon.price) * quantity * addonQty,
  ...
  // EXTRAS ARE NOT TAXED - No tax calculation for extras
...
const calculateItemTotal = (item) => {
  ...
  // Add extra prices (NO TAX ON EXTRAS - just add the price)
  if (item.extras) {
...
const updateCartTotals = (state) => {
  ...
  state.items.forEach(item => {
    ...
    // Add extra prices (NO TAX ON EXTRAS - just add to subtotal)
    if (item.extras) {
      Object.entries(item.extras).forEach(([extraId, extraQty]) => {
        const extra = item.product.allExtras?.find(e => e.id === parseInt(extraId));
        if (extra && extraQty > 0) {
          const extraPrice = parseFloat(extra.price) * extraQty * item.quantity;
          const extraPriceAfterDiscount = parseFloat(extra.price_after_discount || extra.price) * extraQty * item.quantity;
